...
- **Never re-point a published `V*` tag, even at a byte-identical tree**: `release.yml` computes the Homebrew core formula's `sha256` from `archive/refs/tags/<TAG>.tar.gz` at the moment the tag push triggers it, and GitHub writes the tag's commit SHA into that archive's pax global header (`comment=<sha>`). Moving the tag therefore changes the tarball's checksum while every file stays identical, and the merged formula is silently wrong from then on. V1.54.0 shipped that way: the release commit was rebased onto a `chore: update contributors` push, the tag followed it from `65c98814` to `7b559efd` (same tree `ddb90a20`), and `brew upgrade mole` failed the source checksum for everyone who had to build from source, which since Homebrew dropped Intel bottles is every Intel user (#1591). Push the branch first, let the bot settle, then tag the SHA that is already on `origin/main`. A tag that must move is a pulled release, so use the next entry and a new version number. Recovery on a live release is a one-line `sha256` PR to `Homebrew/homebrew-core` with the current tarball's hash; the bottles are unaffected because BrewTestBot built them from the original download.
