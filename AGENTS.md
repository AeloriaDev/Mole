...
- **A new cleanup target needs measured value and an explicit non-target list.** State bytes actually reclaimable on a real app version, not just the target's total footprint; name sibling directories excluded as user data and prove protection covers every reachable cleanup path. "It looks like a cache" is not evidence, and zero measured value stays out of scope. An encrypted or opaque index cannot prove a directory is unreferenced, so exclude it. A third-party owner command is still a deletion sink: accept it only when the supported release exposes every mutated root machine-readably, dry-run and real mode share one candidate plan, one lock covers the complete mutation rather than one phase, downstream traversal enforces no-follow physical containment, and partial failures are observable. Mole must validate and whitelist both lexical and physical roots, bind those identities again at the command boundary, propagate timeout or signal cancellation, and never fall back to direct deletion.
- **A dependency store is not a cache; only redundant copies get cleaned.** The test is whether the toolchain resolves against the directory itself or against a copy already materialized elsewhere. `~/.cargo/registry/cache` holds `.crate` tarballs whose contents are already extracted into `registry/src`, so it goes; `registry/src`, `~/go/pkg/mod`, `$DENO_DIR`, `~/.ivy2/cache`, `~/.m2/repository`, `~/.nuget/packages`, `~/.cabal/packages`, and `~/.cpan/sources` are what builds read, so they stay and `clean_large_files` reports them for review instead. The same applies to anything whose restoration is a download rather than a local rebuild: downloaded model weights and run artifacts (`~/.cache/huggingface`, `~/.cache/torch`, `~/.cache/tensorflow`, `~/.cache/wandb`) and toolchain payloads (`~/.sbt/boot`, `~/.sbt/launchers`, `~/.stack/programs`) are off the delete path entirely. Two traps this area has already sprung: a name ending in `cache` proves nothing, since `~/.ivy2/cache` and `~/.nuget/packages` are both stores; and `DEFAULT_WHITELIST_PATTERNS` is not protection, because `lib/core/base.sh` replaces it wholesale the moment a user saves their own whitelist file, which is how `~/.cache/torch` shipped deletable for everyone while its sibling `~/.cache/huggingface` looked covered. Fix the default path rather than adding a whitelist row, and add a Bats case that fails on the old code.
