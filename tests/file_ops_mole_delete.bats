...
@test "mole_truncate_log_file empties a live log instead of unlinking it" {
    run /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
...
tmp=$(mktemp -d)
log="$tmp/agent.log"
head -c 4096 /dev/zero > "$log"
validate_path_for_deletion() { return 0; }

# A writer holding the descriptor is the whole point: unlink would free no
# blocks and a path-reopening writer would then crash.
exec 9>> "$log"
mole_truncate_log_file "$log" || { echo "WRONG: truncate failed"; exit 1; }
exec 9>&-

[[ -f "$log" ]] || { echo "WRONG: file was unlinked, not emptied"; exit 1; }
[[ ! -s "$log" ]] || { echo "WRONG: file still has content"; exit 1; }
echo OK
EOF
    [ "$status" -eq 0 ]
    [[ "$output" == *"OK"* ]]
...
@test "mole_truncate_log_file refuses symlinks, non-regular files and non-logs" {
    run /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
...
# A symlink could redirect the truncate at a file outside the reviewed tree.
echo secret > "$tmp/target.log"
ln -s "$tmp/target.log" "$tmp/link.log"
if mole_truncate_log_file "$tmp/link.log"; then echo "WRONG: followed symlink"; exit 1; fi
[[ -s "$tmp/target.log" ]] || { echo "WRONG: symlink target was emptied"; exit 1; }

# Config and state files live right next to logs.
for name in app.log.json app.log.lock settings.plist notes.txt; do
    echo data > "$tmp/$name"
    if mole_truncate_log_file "$tmp/$name"; then echo "WRONG: emptied $name"; exit 1; fi
    [[ -s "$tmp/$name" ]] || { echo "WRONG: $name lost content"; exit 1; }
done

# Directories and fifos must never be reachable.
mkdir -p "$tmp/dir.log"
if mole_truncate_log_file "$tmp/dir.log"; then echo "WRONG: emptied a directory"; exit 1; fi
mkfifo "$tmp/pipe.log"
if mole_truncate_log_file "$tmp/pipe.log"; then echo "WRONG: emptied a fifo"; exit 1; fi
echo OK
EOF
    [ "$status" -eq 0 ]
    [[ "$output" == *"OK"* ]]
...
@test "mole_is_log_like_name accepts rotated logs but not neighbouring state" {
    run /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"

for good in app.log App.LOG foo.crash foo.diag foo.log.1 foo.log.gz foo.log.2025-05-01 foo.log.1.gz; do
    mole_is_log_like_name "$good" || { echo "WRONG: rejected $good"; exit 1; }
done
for bad in .hidden.log app.log.json app.log.lock app.log.state app.log.sqlite3 notes.txt app.plist app.log.wal app.log.data app.log.keystore app.log.bak; do
    if mole_is_log_like_name "$bad"; then echo "WRONG: accepted $bad"; exit 1; fi
done
echo OK
EOF
    [ "$status" -eq 0 ]
...
@test "mole_truncate_log_file bound truncate refuses a leaf swapped to a symlink" {
    run /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"

tmp=$(mktemp -d)
echo secret > "$tmp/victim.txt"
ln -s "$tmp/victim.txt" "$tmp/swap.log"

# Calling the bound layer directly models a rename racing in AFTER the
# shell-level [[ -L ]] check: at write time the leaf IS a symlink, and a
# plain `: > path` redirect would follow it. O_NOFOLLOW must refuse.
if _mole_truncate_bound "$tmp/swap.log"; then echo "WRONG: bound truncate followed the swapped symlink"; exit 1; fi
[[ -s "$tmp/victim.txt" ]] || { echo "WRONG: symlink target was emptied"; exit 1; }

# And the bound layer still works on the real thing.
echo content > "$tmp/plain.log"
_mole_truncate_bound "$tmp/plain.log" || { echo "WRONG: bound truncate failed on a regular file"; exit 1; }
[[ ! -s "$tmp/plain.log" ]] || { echo "WRONG: regular file kept content"; exit 1; }
echo OK
EOF
    [ "$status" -eq 0 ]
    [[ "$output" == *"OK"* ]]
...
@test "mole_truncate_log_file honors user whitelist and app protection" {
    run /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"
...
# A path the user explicitly whitelisted must survive the truncate funnel
# exactly as it survives the removal funnel.
echo keepme > "$tmp/whitelisted.log"
is_path_whitelisted() { [[ "$1" == *whitelisted* ]]; }
if mole_truncate_log_file "$tmp/whitelisted.log"; then echo "WRONG: whitelist bypassed"; exit 1; fi
[[ -s "$tmp/whitelisted.log" ]] || { echo "WRONG: whitelisted log was emptied"; exit 1; }

echo keepme > "$tmp/guarded.log"
should_protect_path() { [[ "$1" == *guarded* ]]; }
if mole_truncate_log_file "$tmp/guarded.log"; then echo "WRONG: protection bypassed"; exit 1; fi
[[ -s "$tmp/guarded.log" ]] || { echo "WRONG: protected log was emptied"; exit 1; }
echo OK
EOF
    [ "$status" -eq 0 ]
    [[ "$output" == *"OK"* ]]
...
@test "mole_truncate_log_file honors dry-run and the deletion path guard" {
    run /bin/bash --noprofile --norc << 'EOF'
set -euo pipefail
source "$PROJECT_ROOT/lib/core/common.sh"

tmp=$(mktemp -d)
echo content > "$tmp/agent.log"

validate_path_for_deletion() { return 0; }
MOLE_DRY_RUN=1 mole_truncate_log_file "$tmp/agent.log" || { echo "WRONG: dry-run reported failure"; exit 1; }
[[ -s "$tmp/agent.log" ]] || { echo "WRONG: dry-run emptied the file"; exit 1; }

# A protected path must be refused even when the name looks like a log.
validate_path_for_deletion() { return 1; }
if mole_truncate_log_file "$tmp/agent.log"; then echo "WRONG: bypassed path guard"; exit 1; fi
[[ -s "$tmp/agent.log" ]] || { echo "WRONG: protected file was emptied"; exit 1; }
echo OK
EOF
    [ "$status" -eq 0 ]
    [[ "$output" == *"OK"* ]]
